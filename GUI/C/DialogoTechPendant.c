...
int RobotGoto(HWND hDlg,tAxisSettings *axisSettings,int *axisOP,int Caso)
{
	...
	if(Caso==2) //ORIGEN
	{
		...
		Configurar_techTraj(&pTECH,&axisSettings[0],-80,400,300,AxisPitch);
		pTECH.invalidmask=pTECH.invalidmask+PitchMask;
		//ROLL				
		Configurar_techTraj(&pTECH,&axisSettings[0],0,400,300,AxisRoll);
		pTECH.invalidmask=pTECH.invalidmask+RollMask;
...
void Put_ID(HWND hDlg,int16* pvectorIDs,int ComboxID)
{
	...
	CPoses=pvectorIDs[0];
       /*Borra el combo box cuando se utiliza el boton borrar poses*/
	if(pvectorIDs[1]==-1)
	{
