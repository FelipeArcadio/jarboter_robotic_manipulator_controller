...
proc help::init {filename {topic contents} {parent {}} {width 900} {height 580} } {
#/sch
 variable w
 variable gw
 variable t
 ...
 if {![info exists w] || ![winfo exists $w]} {
   ...
   wm title .tophelpwindow "Robomosp Help System"
  ...
  if {$w!="."} {append w "."}
     # Contents frame
     frame ${w}f -width [ expr $width/3 ] -height $height -bd 2 -relief sunken
     # Text frame
     frame ${w}f1 -width [ expr $width/2 + 10 ] -height $height -bd 2 -relief sunken
     pack ${w}f -side left -fill both -expand true -padx 0 -pady 0
     pack ${w}f1 -side right -fill both -expand true -padx 0 -pady 0
     append w "f"
     append t "${w}1"
     set gw $w
     set gt $t
  ...
  scrollbar $w.vs -command "$w.text yview" -orient vertical -width 10
  scrollbar $w.hs -command "$w.text xview" -orient horizontal -width 10

  # Text Canvas
  canvas $t.title -bd 0 -highlightthickness 0 -cursor arrow -height 0
  text $t.text -wrap none -cursor arrow -state disabled -bd 0 \
               -insertwidth 0 -insertontime 0 -exportselection 1\
               -highlightthickness 0 -takefocus 1 -insertborderwidth 0 \
               -yscrollcommand "$t.vs set" -xscrollcommand "$t.hs set" \
               -foreground "black"\
               -background "white"
    foreach elem {text title} {
   catch {$t.$elem configure -font "-family $font"}
   catch {$t.$elem configure -font "-size $fontsize"}
   catch {$t.$elem configure -foreground $settings(fore)}
   catch {$t.$elem configure -background $settings(back)}
  }

  scrollbar $t.vs -command "$t.text yview" -orient vertical -width 10
  scrollbar $t.hs -command "$t.text xview" -orient horizontal -width 10

  grid rowconfig    $t 0 -weight 1    -minsize 0
  grid rowconfig    $t 1 -weight 1000 -minsize 50
  grid columnconfig $t 50 -weight 1000 -minsize 50

  grid $t.title -padx 0 -pady 0 \
               -row 0 -column 0 -rowspan 1 -columnspan 52 -sticky new
  grid $t.text -padx 0 -pady 0 \
               -row 1 -column 0 -rowspan 1 -columnspan 51 -sticky news
  grid $t.vs   -padx 1 -pady 0 \
               -row 1 -column 51 -rowspan 1 -columnspan 1 -sticky news
  grid $t.hs   -padx 0 -pady 1 \
               -row 2 -column 0 -rowspan 1 -columnspan 51 -sticky news
  #grid $t.d    -padx 0 -pady 0 -row 2 -column 51 -rowspan 1 -columnspan 1 -sticky news
  #grid $t.stat -padx 0 -pady 0 -row 3 -column 50 -rowspan 1 -columnspan 1 -sticky nwes
  #grid $t.ugol -padx 0 -pady 0 -row 3 -column 51 -rowspan 1 -columnspan 1 -sticky es
  grid propagate $t 0
...
#  grid rowconfig    $w 3 -minsize 20
  ...
  bind $w.text <Left>  "$t.text xview scroll -1 units; break"
