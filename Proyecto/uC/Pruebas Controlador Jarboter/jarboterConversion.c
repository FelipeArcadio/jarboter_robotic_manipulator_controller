...
int main(void)
{
	double Radianes,uPasos,grados;
	
	//Variables Menu
	int16 menu,articulacion,submenu,stop,prueba_tipo;	
	//Variables de Movimiento
	PMDint32 position,velocity,aceleration,posicion_roll,posicion_pitch;	
	//Variables Informacion de Movimiento
	PMDint32 position1, velocity1, commandedposition, commandedvelocity,
	commandedacceleration, readencoder1, readerror,readencoder2, readerror1;
	...
	//Variable para la prueba var-up y up-var
	double Angulo;
	int tipo_var;
	...
	//Variables para funciones de griper
	int16 time;
	...
	{  
		...
		switch (menu)
		{
			case (0)://FUNCIONES DE MOVIMIENTO
				...
				{				
					fprintf_P(stdout,PSTR("Digite Posicion en grados? \n"));
					scanf("%f",(float*) &grados);		
					printf("Valor introducido =	%.2f \n",grados);
					Radianes=grados2rad(grados);
					uPasos=rad2usteps(&hAxis[articulacion].axisSettings,Radianes);
					position=positionPCtoUC(uPasos);  
					printf("posicion_en uPasos =	%08ld \n",position);
					
					fprintf_P(stdout,PSTR("Digite Velocidad en grados? \n"));
					scanf("%f", (float*)&grados);	
					Radianes=grados2rad(grados);
					uPasos=rad2usteps(&hAxis[articulacion].axisSettings,Radianes);
					velocity=velocityPCtoUC(uPasos); 
					printf("velocity_en uPasos =	%08ld \n",velocity);
					
					fprintf_P(stdout,PSTR("Digite aceleracion en grados? \n"));
					scanf("%f", (float*)&grados);
					Radianes=grados2rad(grados);
					uPasos=rad2usteps(&hAxis[articulacion].axisSettings,Radianes);
					aceleration=accelerationPCtoUC(uPasos); 
					printf("aceleration_en uPasos =	%08ld \n",aceleration);
				}
		    ...
		    case (1)://FUNCIONES DE INFORMACION DE MOVIMIENTO
				...
				fprintf_P(stdout,PSTR("5. Ajustar valor encoder \n"));
				fprintf_P(stdout,PSTR("6. Clear position error \n"));
				fprintf_P(stdout,PSTR("7. Prueba Mov Roll_pich \n"));
				...
				switch (submenu)
				{
				    ...
				    break;
					case (5):
						fprintf_P(stdout,PSTR("Digite el valor a cargar en el encoder \n"));
						scanf("%d",&P_ajuste);
						cargar_encoder(articulacion,P_ajuste);
				    break;
					
					case (6):
						PMDClearPositionError(&hAxis[articulacion]);
						PMDUpdate(&hAxis[articulacion]);
					break;
					
					
					case (7):
					
						fprintf_P(stdout,PSTR("Casos_de prueba\n"));
						fprintf_P(stdout,PSTR("1. Motor Pitch + y Motor Roll + \n"));
						fprintf_P(stdout,PSTR("2. Motor Pitch + y Motor Roll - \n"));
						fprintf_P(stdout,PSTR("3. Motor Pitch - y Motor Roll + \n"));
						fprintf_P(stdout,PSTR("4. Motor Pitch - y Motor Roll - \n"));
						scanf("%d",&prueba_tipo);
						
						
						PMDGetActualPosition(&hAxis[AxisPitch],&readencoder1);
						PMDGetPositionError(&hAxis[AxisPitch],& readerror);	
						PMDGetActualPosition(&hAxis[AxisRoll],&readencoder2);
						PMDGetPositionError(&hAxis[AxisRoll],& readerror1);	

						printf("Valor encoder de pitch = %08ld valor encoder de roll = = %08ld\n",readencoder1,readencoder2);
						printf("Valor error de pitch = %08ld valor error de roll = = %08ld\n",readerror,readerror1);
						
						fprintf_P(stdout,PSTR("Digite Posicion en grados siempre + ? \n"));
						scanf("%f",(float*) &grados);		
						printf("Valor introducido =	%.2f \n",grados);
						Radianes=grados2rad(grados);
						//siempre se utiliza la relacion de pitch, es indiferentes pues esta
						//en 1 en ambos y son los mismos tipos de motores
						uPasos=rad2usteps(&hAxis[AxisPitch].axisSettings,Radianes);
						position=positionPCtoUC(uPasos);  
						printf("posicion_en uPasos =	%08ld \n",position);
						...
						{
						    case (1):
								posicion_pitch = position ;
								posicion_roll  = position ;
						    break;						
						    case (2):
								posicion_pitch = position ;
								posicion_roll  = -position ;
						...
						printf("Ejecutando prueba motor Pitch = %08ld y motor roll = %08ld \n",posicion_pitch,posicion_roll);
						mov_axis(AxisPitch,posicion_pitch,velocity,aceleration,update);
						mov_axis(AxisRoll,posicion_roll,velocity,aceleration,update);
						//tengo esperar a que el motor se detenga.
						scanf("%d",&stop);
						articulacion=AxisPitch;
						infomov(articulacion,&position1, &velocity1, &acceleration1, &deceleration, 
								&commandedposition,	&commandedvelocity, &commandedacceleration, 
								&readencoder1, &readerror,&cuentas_encoder,&micropasos);
						printf("la informacion correspondiente del movimiento del eje %s es :\n",AxisName(articulacion));
						printf("posicion=	%08ld\n",position1);
						printf("velocidad	= 	%08ld\n",velocity1);
						printf("posicion instantanea=	%08ld\n",commandedposition);
						printf("velodidad instantanea=	%08ld\n",commandedvelocity);
						printf("Lectura Encoder=	%08ld\n",readencoder1);
						printf("Error Posicion=	%08ld\n",readerror);	
						scanf("%d",&stop);		
						articulacion=AxisRoll;
						infomov(articulacion,&position1, &velocity1, &acceleration1, &deceleration, 
								&commandedposition,	&commandedvelocity, &commandedacceleration, 
								&readencoder1, &readerror,&cuentas_encoder,&micropasos);
						printf("la informacion correspondiente del movimiento del eje %s es :\n",AxisName(articulacion));
						...
						printf("velocidad	= 	%08ld\n",velocity1);
						printf("posicion instantanea=	%08ld\n",commandedposition);
						printf("velocidad instantanea=	%08ld\n",commandedvelocity);
						printf("Lectura Encoder=	%08ld\n",readencoder1);
						printf("Error Posicion=	%08ld\n",readerror);	
						scanf("%d",&stop);		
					break;
			...
			case(7)://FUNCIONES DE GRIPPER
				...
				fprintf_P(stdout,PSTR("2. Abrir Gripper con tiempo \n"));
				fprintf_P(stdout,PSTR("3. Cerrar Gripper con tiempo \n"));
				fprintf_P(stdout,PSTR("Otra Opcion: Salir SubMenu \n"));
				scanf("%d",&submenu);
				...
				switch (submenu)
				{
					...
					case (2):
						fprintf_P(stdout,PSTR("Digite el tiempo de apertura del gripper \n"));
						scanf("%d",&time);
						openGripper_time(time); 
				    break;
					
					case (3):
						fprintf_P(stdout,PSTR("Digite el tiempo de cerrado del gripper \n"));
						scanf("%d",&time);
						closeGripper_time(time); 
				    break;
			...
			case(8)://Funciones para DEBUG
				...
				fprintf_P(stdout,PSTR("3. Grados_to_upasos \n"));
				fprintf_P(stdout,PSTR("4. U_pasos_to_grados \n"));
				fprintf_P(stdout,PSTR("Otra Opcion: Salir SubMenu \n"));
				scanf("%d",&submenu);		
				...
				{	
					fprintf_P(stdout,PSTR("Tipo de variable: \n"));
					fprintf_P(stdout,PSTR("0. Posicion. \n"));
					fprintf_P(stdout,PSTR("1. Velocidad. \n"));
					fprintf_P(stdout,PSTR("2. Aceleracion. \n"));
					scanf("%d",&tipo_var);
				}			
				...
				switch (submenu)
				{
					...
					case (3):
						fprintf_P(stdout,PSTR("Digite el angulo\n"));
						scanf("%f",(float*)&Angulo);
						switch (tipo_var)
						{
							case (0):
								printf("posicion_en uPasos =	%08ld \n",
								gradrosTOuPasos(&hAxis[articulacion].axisSettings,Angulo,POSICION_TYPE));
							break;
							case (1):
								printf("Velocidad  en uPasos =	%08ld \n",
								gradrosTOuPasos(&hAxis[articulacion].axisSettings,Angulo,VELOCIDAD_TYPE));
							break;
							case (2):
								printf("aceleration_en uPasos =	%08ld \n",
								gradrosTOuPasos(&hAxis[articulacion].axisSettings,Angulo,ACELERACION_TYPE));
							break;
						}
				    break;
					case (4):
						fprintf_P(stdout,PSTR("Digite el valor\n"));
						scanf("%ld", &position);
						switch (tipo_var)
						{
							case (0):	
								printf("posicion_en Grados =%f \n",
								uPasosTOgradros(&hAxis[articulacion].axisSettings,position,POSICION_TYPE));
							break;
							case (1):
								printf("Velocidad_en Grados =%f \n",
								uPasosTOgradros(&hAxis[articulacion].axisSettings,position,VELOCIDAD_TYPE));
							break;
							case (2):
								printf("posicion_en Grados =%f \n",
								uPasosTOgradros(&hAxis[articulacion].axisSettings,position,ACELERACION_TYPE));
							break;
						}
				    break;
					default:
